...
export const BatchQueue: React.FC<BatchQueueProps> = ({
  ...
  return (
    <div id="batch-queue-container" className="bg-white rounded-2xl p-4 sm:p-5 border border-zinc-200 shadow-sm space-y-4">
        ...
        <div className="flex items-center gap-2">
          {pendingCount > 0 && (
            <button
              ...
              onClick={onRunBatch}
              className="flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-semibold rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm transition-colors disabled:opacity-50"
            >
              {isProcessingBatch ? (
                <>
          ...
          {completedCount > 0 && (
            <button
              ...
              type="button"
              onClick={onExportCsv}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg border border-zinc-300 text-zinc-700 hover:bg-zinc-50 transition-colors"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 max-h-72 overflow-y-auto pr-1">
        {items.map((item) => {
          ...
          return (
            ...
            >
              {/* Thumbnail */}
              <div className="w-12 h-12 rounded-lg bg-zinc-200 overflow-hidden shrink-0 relative">
                <img
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  {item.status === 'completed' && (
                    <span className="inline-flex items-center gap-1 text-[10px] font-medium text-emerald-700 bg-emerald-100/60 px-1.5 py-0.2 rounded">
              ...
              <button
                ...
                className="text-zinc-300 hover:text-zinc-500 p-1 rounded-md hover:bg-zinc-100 transition-colors"
