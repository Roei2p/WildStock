...
export const N8nWorkflowModal: React.FC<N8nWorkflowModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden shadow-2xl flex flex-col border border-zinc-200">
        {/* Modal Header */}
        ...
        <div className="p-5 sm:p-6 overflow-y-auto space-y-5 text-zinc-800">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-zinc-700 transition-colors"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-xs font-semibold px-3 py-1 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white transition-colors shadow-sm"
                >
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-zinc-600 flex items-center gap-1">
              ...
              <button
                ...
                className="flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-zinc-700 transition-colors"
              >
                {curlCopied ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5" />}
                <span>{curlCopied ? 'Copied' : 'Copy cURL'}</span>
              </button>
            </div>
            <pre className="p-3 bg-zinc-900 text-emerald-400 font-mono text-xs rounded-xl overflow-x-auto border border-zinc-800">
              {curlCommand}
            </pre>
          </div>
          ...
          <div className="p-4 rounded-xl border border-zinc-200 bg-zinc-50 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] font-mono text-zinc-500">{webhookUrl}</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 text-xs px-3 py-1.5 rounded-lg border border-zinc-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white"
              />
              <button
                type="button"
                disabled={isTesting || !testUrl}
                onClick={handleRunTestWebhook}
                className="px-3.5 py-1.5 text-xs font-semibold rounded-lg bg-zinc-900 hover:bg-zinc-800 text-white disabled:opacity-50 flex items-center gap-1.5 transition-colors"
              >
                {isTesting ? (
                  <>
            ...
            {testResult && (
              <div className="mt-3">
                ...
                <pre className="p-3 bg-zinc-950 text-emerald-400 font-mono text-xs rounded-lg overflow-x-auto border border-zinc-800 max-h-48">
                  {testResult}
                </pre>
              </div>
            )}
